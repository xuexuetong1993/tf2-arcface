...
flags.DEFINE_string('dataset_path', '/root/paddlejob/workspace/env_run/xuetong/tf2-arcface/arcface-tf2/data/GLDv2/',
                    'path to dataset')
flags.DEFINE_string('output_path', './GLDv2/gldv2_clean_shuffle.tfrecord',
...
def main(_):
    ...
    fin = open(os.path.join(FLAGS.dataset_path,"./train_list.txt"), 'r')
    """
    fin = open(os.path.join(FLAGS.dataset_path,"./train_list.txt"), 'r')
    for idx, line in enumerate(fin):
        data= line.strip().split(" ")
        id_name = int(data[1])
        img_path = os.path.join(FLAGS.dataset_path, data[0])
        file_name = line.strip() 
        samples.append((img_path, id_name, file_name))
        #print(samples[idx][0], samples[idx][1])
        if idx % 10000 == 0:
            print("process : ", idx)
