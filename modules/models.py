...
def Backbone(backbone_type='ResNet50', use_pretrain=True):
    ...
    if use_pretrain:
        weights = 'imagenet'
        print("------------------- use imagenet weights")
    def backbone(x_in):
        #print("x_in.shape:", x_in.shape, x_in.shape[1:])
...
def ArcFaceModel(size=None, channels=3, num_classes=None, name='arcface_model',
    ...
    #x_train = tf.keras.layers.experimental.preprocessing.CenterCrop(112,112)(x_train)
    ...
    #print("Outputlayer out:", embds)
    if training:
        assert num_classes is not None
        labels = Input([], name='label')
        #print("Input archead: x,y", embds, labels)
